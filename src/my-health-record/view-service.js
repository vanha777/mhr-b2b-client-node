...
let getView = ({product, user, organisation}, patient, viewOptions) => {
	...
	if (viewOptions.view === "hro"){
		...
		viewParametersXml = `<q1:fromDate>2024-01-01</q1:fromDate>
		<q1:toDate>2024-04-01</q1:toDate>`;
		type = "cda";
		...
		type = "cda";
		viewParametersXml = `<q1:fromDate>2012-02-04</q1:fromDate>
	...
	try {
		executeRequest(organisation, "getView", 
			...
			(error, response, body) => {
				...
				fs.writeFile("C:\\Users\\EricAlforque\\Desktop\\testcases\\Test 126 Request.xml", response.request.body.replace('\n', '').replace('\r', ''), function(err) {
					if(err) {
				...
				fs.writeFile("C:\\Users\\EricAlforque\\Desktop\\testcases\\Test 126 Response.xml", xopRes, function(err) {
					if(err) {
						return console.log(err);
					}
				}); 

				fs.writeFile("C:\\Users\\EricAlforque\\Desktop\\testcases\\Test 126 Body.xml", body, function(err) {
					if(err) {
						return console.log(err);
					}
				}); 
				...
				zip.generateNodeStream({ type: 'nodebuffer', streamFiles: true })
					.pipe(fs.createWriteStream('C:\\Users\\EricAlforque\\Desktop\\testcases\\CDAPackage.zip'))
					.on('finish', () => {
