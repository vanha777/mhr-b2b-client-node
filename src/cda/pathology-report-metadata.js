...
let pathologyReportMetadata = (documentObject, package) => {
	...
	return {
		...
		serviceStartTime: 		moment(documentDetails.report.tests.map(test => test.collectionDate).reduce((latestDate, date, ) => latestDate.MeasureDate > date.MeasureDate ? latestDate : date)).format("YYYYMMDDHHmmss"),
		serviceStopTime: 		moment(documentDetails.report.tests.map(test => test.collectionDate).reduce((latestDate, date, ) => latestDate.MeasureDate > date.MeasureDate ? latestDate : date)).format("YYYYMMDDHHmmss"),
